...
func (a *AIAssistant) ProcessRequest(req *AIRequest) (*AIResponse, error) {
	...
	case TaskWrite:
		response.Content = "[AI写作] 基于上下文生成的文档内容"
	case TaskTranslate:
		response.Content = "[AI翻译] 翻译结果"
	case TaskCodeReview:
		response.Content = "[AI代码审查] 代码审查意见"
	case TaskDataAnalysis:
		response.Content = "[AI数据分析] 数据分析报告"
	case TaskEmailDraft:
		response.Content = "[AI邮件] 邮件草稿"
	case TaskDocSearch:
		response.Content = "[AI文档搜索] 搜索结果"
	}
	return response, nil
}
