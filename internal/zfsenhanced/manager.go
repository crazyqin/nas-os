...
type Manager struct {
	...
	scrubPolicies     map[string]*ScrubSchedulePolicy
	scrubJobs         map[string]*ScrubJob
	snapshotPolicies  map[string]*SnapshotLifecyclePolicy
	snapshotTemplates map[string]*SnapshotTemplate
	repairQueue       *RepairQueue
	repairTasks       map[string]*AutoRepairTask
	realtimeHistory   map[string][]RealtimeMetrics
	poolAnalyses      map[string]*PoolAnalysis
}
...
func (m *Manager) CreateScrubPolicy(policy *ScrubSchedulePolicy) (*ScrubSchedulePolicy, error) {
	...
	if policy.IntervalDays <= 0 {
		policy.IntervalDays = 14
	}
	if policy.Priority == 0 {
		policy.Priority = ScrubPriorityNormal
	}
	...
	if policy.Strategy == ScrubStrategyTimed && policy.IntervalDays > 0 {
		...
		if policy.PreferredHour > 0 {
			next = time.Date(next.Year(), next.Month(), next.Day(), policy.PreferredHour, 0, 0, 0, next.Location())
		}
		policy.NextScrub = &next
	}
	m.scrubPolicies[policy.ID] = policy
	return policy, nil
}
func (m *Manager) UpdateScrubPolicy(id string, update *ScrubSchedulePolicy) (*ScrubSchedulePolicy, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	policy, ok := m.scrubPolicies[id]
	if !ok {
	...
	if update.Strategy != "" {
		policy.Strategy = update.Strategy
	}
	if update.IntervalDays > 0 {
		policy.IntervalDays = update.IntervalDays
	}
	if update.Priority > 0 {
		policy.Priority = update.Priority
	}
	policy.BandwidthMBps = update.BandwidthMBps
	policy.IOThreshold = update.IOThreshold
	policy.AutoPause = update.AutoPause
...
func (m *Manager) DeleteScrubPolicy(id string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	if _, ok := m.scrubPolicies[id]; !ok {
		return fmt.Errorf("策略 %s 不存在", id)
	}
	delete(m.scrubPolicies, id)
	return nil
}
...
func (m *Manager) RunScrub(ctx context.Context, poolName string, priority ScrubPriority) (*ScrubJob, error) {
	m.mu.RLock()
	_, exists := m.poolMgr.pools[poolName]
	m.mu.RUnlock()
	if !exists {
		if _, err := m.poolMgr.GetPoolStatus(ctx, poolName); err != nil {
			return nil, fmt.Errorf("池 %s 不存在", poolName)
		}
	}
	job := &ScrubJob{ID: uuid.New().String(), PoolName: poolName, Status: ScrubStatusRunning, StartTime: time.Now()}
	m.mu.Lock()
	m.scrubJobs[job.ID] = job
	m.mu.Unlock()
	...
	job.EndTime = &now
	if err != nil {
		job.Status = ScrubStatusFailed
		job.ErrorMsg = err.Error()
	} else {
		job.Status = ScrubStatusCompleted
		job.Errors = result.ErrorsFound
		job.Repaired = result.RepairsMade
		job.Progress = 100
	}
	m.mu.Unlock()
	return job, nil
}
func (m *Manager) CancelScrub(ctx context.Context, poolName string) error {
	return m.poolMgr.CancelScrub(ctx, poolName)
}
func (m *Manager) GetScrubJobs(poolName string) []*ScrubJob {
	m.mu.RLock()
	defer m.mu.RUnlock()
	jobs := make([]*ScrubJob, 0)
	for _, j := range m.scrubJobs {
		if poolName == "" || j.PoolName == poolName {
			cp := *j
			jobs = append(jobs, &cp)
		}
	}
...
func (m *Manager) CheckAndRunScheduledScrubs(ctx context.Context) ([]string, error) {
	m.mu.RLock()
	var toRun []*ScrubSchedulePolicy
	for _, p := range m.scrubPolicies {
		if !p.Enabled || p.Strategy == ScrubStrategyManual {
			continue
		}
		if p.NextScrub != nil && time.Now().After(*p.NextScrub) {
			cp := *p
	...
	for _, policy := range toRun {
		if policy.Strategy == ScrubStrategySmart && policy.IOThreshold > 0 {
			...
			if err == nil && (metrics.ReadIOPS+metrics.WriteIOPS) > int64(policy.IOThreshold) {
				continue
			}
		}
		...
		if err == nil {
			...
			m.mu.Lock()
			if p, ok := m.scrubPolicies[policy.ID]; ok {
				now := time.Now()
				p.LastScrub = &now
...
func (m *Manager) CreateSnapshotLifecycle(policy *SnapshotLifecyclePolicy) (*SnapshotLifecyclePolicy, error) {
	...
	if len(policy.Datasets) == 0 {
		return nil, fmt.Errorf("数据集列表不能为空")
	}
	if policy.TemplateName != "" {
		if tmpl, ok := m.snapshotTemplates[policy.TemplateName]; ok {
			if policy.RetentionCount == 0 {
				policy.RetentionCount = tmpl.RetentionCount
			}
			if policy.RetentionDays == 0 {
				policy.RetentionDays = tmpl.RetentionDays
			}
			if policy.Schedule == "" {
				policy.Schedule = tmpl.Schedule
			}
			policy.Recursive = tmpl.Recursive
		}
	}
	if policy.RetentionCount <= 0 {
	...
	policy.CreatedAt = now
	policy.UpdatedAt = now
	next := now.Add(time.Duration(policy.IntervalMinutes) * time.Minute)
	policy.NextRun = &next
	m.snapshotPolicies[policy.ID] = policy
	return policy, nil
}
func (m *Manager) UpdateSnapshotLifecycle(id string, update *SnapshotLifecyclePolicy) (*SnapshotLifecyclePolicy, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	policy, ok := m.snapshotPolicies[id]
	if !ok {
		return nil, fmt.Errorf("策略 %s 不存在", id)
	}
	if update.Name != "" {
	...
	if len(update.Datasets) > 0 {
		policy.Datasets = update.Datasets
	}
	if update.IntervalMinutes > 0 {
		policy.IntervalMinutes = update.IntervalMinutes
	}
	if update.RetentionCount > 0 {
	...
	if update.Prefix != "" {
		policy.Prefix = update.Prefix
	}
	policy.Recursive = update.Recursive
	policy.Enabled = update.Enabled
	policy.AutoCreate = update.AutoCreate
	policy.AutoDestroy = update.AutoDestroy
	policy.UpdatedAt = time.Now()
	return policy, nil
}
func (m *Manager) DeleteSnapshotLifecycle(id string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	if _, ok := m.snapshotPolicies[id]; !ok {
		return fmt.Errorf("策略 %s 不存在", id)
	}
	delete(m.snapshotPolicies, id)
	return nil
}
func (m *Manager) ListSnapshotLifecycles() []*SnapshotLifecyclePolicy {
	...
	for _, p := range m.snapshotPolicies {
		cp := *p
		policies = append(policies, &cp)
	}
	return policies
...
func (m *Manager) ExecuteSnapshotLifecycle(ctx context.Context, policyID string) error {
	m.mu.RLock()
	policy, ok := m.snapshotPolicies[policyID]
	if !ok {
		m.mu.RUnlock()
	...
	if !policy.Enabled {
		m.mu.RUnlock()
		return fmt.Errorf("策略 %s 已禁用", policyID)
	}
	policyCopy := *policy
	m.mu.RUnlock()
	var lastErr error
	snapshotName := fmt.Sprintf("%s%s", policyCopy.Prefix, time.Now().Format("20060102-150405"))
	for _, dataset := range policyCopy.Datasets {
		if err := m.poolMgr.CreateSnapshot(ctx, dataset, snapshotName, policyCopy.Recursive); err != nil {
			lastErr = err
		}
	}
	if policyCopy.AutoDestroy {
		for _, dataset := range policyCopy.Datasets {
			m.cleanupDatasetSnapshots(ctx, policyCopy.PoolName, dataset, policyCopy.Prefix, policyCopy.RetentionCount, policyCopy.RetentionDays)
		}
	}
	m.mu.Lock()
	if p, exists := m.snapshotPolicies[policyID]; exists {
		now := time.Now()
		p.LastRun = &now
...
func (m *Manager) CheckAndRunSnapshotLifecycles(ctx context.Context) ([]string, error) {
	m.mu.RLock()
	var toRun []string
	for id, p := range m.snapshotPolicies {
		if !p.Enabled || !p.AutoCreate {
			continue
		}
		if p.NextRun != nil && time.Now().After(*p.NextRun) {
	...
	for _, id := range toRun {
		if err := m.ExecuteSnapshotLifecycle(ctx, id); err == nil {
...
func (m *Manager) ListSnapshotTemplates() []SnapshotTemplate {
	...
	for _, t := range m.snapshotTemplates {
...
func (m *Manager) DetectAndRepair(ctx context.Context, poolName string) ([]*AutoRepairTask, error) {
	...
	if len(tasks) > 0 {
		go m.processRepairQueue(context.Background())
	}
	return tasks, nil
}
...
func (m *Manager) GetRepairTasks(poolName string) []*AutoRepairTask {
	...
	for _, t := range m.repairTasks {
		if poolName == "" || t.PoolName == poolName {
			cp := *t
...
func (m *Manager) GetRealtimeMetrics(ctx context.Context, poolName string) (*RealtimeMetrics, error) {
	...
	perfMetrics, err := m.perfMonitor.CollectMetrics(ctx, poolName)
	if err != nil {
		return nil, err
	}
	...
	m.mu.Lock()
	m.realtimeHistory[poolName] = append(m.realtimeHistory[poolName], *metrics)
	if len(m.realtimeHistory[poolName]) > 1000 {
		m.realtimeHistory[poolName] = m.realtimeHistory[poolName][len(m.realtimeHistory[poolName])-1000:]
	}
	m.mu.Unlock()
	return metrics, nil
}
func (m *Manager) GetMetricsHistory(poolName string, limit int) []RealtimeMetrics {
	m.mu.RLock()
	defer m.mu.RUnlock()
	history, exists := m.realtimeHistory[poolName]
	if !exists {
		return nil
	}
	...
	result := make([]RealtimeMetrics, limit)
...
func (m *Manager) AnalyzePool(ctx context.Context, poolName string) (*PoolAnalysis, error) {
	...
	if err == nil {
		analysis.CapacityTrend = *capTrend
		analysis.DailyGrowthBytes = capTrend.GrowthRateDay
		if capTrend.DaysUntilFull > 0 {
			fullDate := time.Now().AddDate(0, 0, capTrend.DaysUntilFull)
			analysis.PredictedFullDate = &fullDate
		}
	}
	compStats, err := m.perfMonitor.GetCompressionStats(ctx, poolName, poolName)
	if err == nil {
		analysis.CompressionStats = compStats
	}
	dedupStats, err := m.perfMonitor.GetDedupStats(ctx, poolName)
	if err == nil {
		analysis.DedupStats = dedupStats
	}
	for _, disk := range pool.Disks {
		summary := DiskHealthSummary{Name: disk.Name, Status: string(disk.Status)}
		if disk.SMART != nil {
			summary.Temperature = disk.SMART.Temperature
			summary.PowerOnHours = disk.SMART.PowerOnHours
			summary.Reallocated = disk.SMART.ReallocatedSectors
			summary.HealthPercent = m.calculateDiskHealthPercent(disk.SMART)
		}
	...
	m.mu.Lock()
	m.poolAnalyses[poolName] = analysis
	m.mu.Unlock()
	return analysis, nil
}
func (m *Manager) GetPoolAnalysis(poolName string) (*PoolAnalysis, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	analysis, ok := m.poolAnalyses[poolName]
	if !ok {
		return nil, fmt.Errorf("池 %s 没有分析数据", poolName)
	}
	return analysis, nil
}
func (m *Manager) GetCapacityTrend(ctx context.Context, poolName string) (*CapacityTrend, error) {
	return m.perfMonitor.GetCapacityTrend(ctx, poolName)
}
...
func (m *Manager) GetPoolStatus(ctx context.Context, name string) (*PoolInfo, error) {
	return m.poolMgr.GetPoolStatus(ctx, name)
}
func (m *Manager) GetAlerts() []Alert { return m.poolMgr.GetAlerts() }
func (m *Manager) AcknowledgeAlert(alertID, ackedBy string) error {
	return m.poolMgr.AcknowledgeAlert(alertID, ackedBy)
}
...
func (m *Manager) cleanupDatasetSnapshots(ctx context.Context, poolName, dataset, prefix string, maxCount, maxDays int) {
	snapshots, err := m.poolMgr.ListSnapshots(ctx, poolName)
	if err != nil {
		return
	}
	var matched []SnapshotInfo
	for _, snap := range snapshots {
		if snap.Dataset == dataset && len(snap.SnapshotName) > len(prefix) && snap.SnapshotName[:len(prefix)] == prefix {
			matched = append(matched, snap)
		}
	}
	sort.Slice(matched, func(i, j int) bool { return matched[i].CreatedAt.After(matched[j].CreatedAt) })
	if maxCount > 0 && len(matched) > maxCount {
		for _, snap := range matched[maxCount:] {
			_ = m.poolMgr.DeleteSnapshot(ctx, dataset, snap.SnapshotName, false)
		}
	}
	if maxDays > 0 {
...
func (m *Manager) processRepairQueue(ctx context.Context) {
	for m.repairQueue.Len() > 0 {
		task := m.repairQueue.Pop()
		if task == nil {
			break
		}
		m.mu.Lock()
		...
		if err != nil {
			...
			task.RepairedCount = result.RepairsMade
		}
		m.mu.Unlock()
	}
}
func (m *Manager) calculateHealthScore(pool *PoolInfo) float64 {
	...
	case PoolStatusFaulted:
		score -= 60
	case PoolStatusOffline, PoolStatusUnavail:
		score -= 80
	}
	if pool.UsedPercent > 90 {
		score -= 20
	} else if pool.UsedPercent > 80 {
		score -= 10
	}
...
func (m *Manager) calculateDiskHealthPercent(smart *SMARTInfo) float64 {
	...
	if smart.ReallocatedSectors > 0 {
		health -= float64(smart.ReallocatedSectors) * 2
	}
	if smart.PendingSectors > 0 {
		health -= float64(smart.PendingSectors) * 3
	}
	if smart.Temperature > 50 {
		health -= float64(smart.Temperature-50) * 1.5
	}
	if health < 0 {
		health = 0
	}
...
func (m *Manager) generatePoolRecommendations(pool *PoolInfo, analysis *PoolAnalysis) []string {
	var recs []string
	if pool.UsedPercent > 85 {
		recs = append(recs, fmt.Sprintf("池 %s 使用率 %.1f%%，建议扩容或清理数据", pool.Name, pool.UsedPercent))
	}
	if pool.Fragmentation > 30 {
		recs = append(recs, fmt.Sprintf("池 %s 碎片率 %.1f%%，建议执行 scrub 或清理旧快照", pool.Name, pool.Fragmentation))
	}
	if pool.Status != PoolStatusOnline {
		recs = append(recs, fmt.Sprintf("池 %s 状态 %s，建议检查磁盘健康并替换故障盘", pool.Name, pool.Status))
	}
	totalErrors := pool.ReadErrors + pool.WriteErrors + pool.ChecksumErrors
	if totalErrors > 0 {
		recs = append(recs, fmt.Sprintf("池 %s 存在 %d 个错误，建议执行 scrub 检查", pool.Name, totalErrors))
	}
	if analysis.DailyGrowthBytes > 0 && analysis.PredictedFullDate != nil {
		daysLeft := int(time.Until(*analysis.PredictedFullDate).Hours() / 24)
		if daysLeft < 30 {
			recs = append(recs, fmt.Sprintf("按当前增速，池 %s 预计 %d 天后满，建议尽快扩容", pool.Name, daysLeft))
		}
	}
	if len(recs) == 0 {
		recs = append(recs, "池运行状况良好，无需特别操作")
	}
	return recs
}
