...
const (
	StorageTypeSSD     StorageType = "ssd"     // 固态硬盘
	StorageTypeHDD     StorageType = "hdd"     // 机械硬盘
	StorageTypeNVMe    StorageType = "nvme"    // NVMe 高速盘
	StorageTypeTape    StorageType = "tape"    // 磁带归档
	StorageTypeCloud   StorageType = "cloud"   // 云存储
...
const (
	StrategyColdMigration  OptimizationStrategy = "cold_migration"  // 冷数据迁移
	StrategyDeduplication  OptimizationStrategy = "deduplication"   // 去重
	StrategyCompression    OptimizationStrategy = "compression"     // 压缩
	StrategyTiering        OptimizationStrategy = "tiering"         // 自动分层
	StrategyCleanup        OptimizationStrategy = "cleanup"         // 清理过期数据
	StrategyArchivePolicy  OptimizationStrategy = "archive_policy" // 归档策略
)
