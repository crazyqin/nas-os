...
func TestPolicyEngine_AddAndGetPolicy(t *testing.T) {
	...
	if err := pe.AddPolicy(p); err != nil { t.Fatalf("AddPolicy: %v", err) }
	got, err := pe.GetPolicy("p1")
	if err != nil { t.Fatalf("GetPolicy: %v", err) }
	if got.Name != "admin" { t.Errorf("name: %s", got.Name) }
	if got.CreatedAt.IsZero() { t.Error("CreatedAt zero") }
}

func TestPolicyEngine_AddPolicy_Validation(t *testing.T) {
	pe := NewPolicyEngine()
	if err := pe.AddPolicy(&SecurityPolicy{Name: "x"}); err == nil { t.Error("reject empty ID") }
	if err := pe.AddPolicy(&SecurityPolicy{ID: "p1"}); err == nil { t.Error("reject empty name") }
}

func TestPolicyEngine_RemovePolicy(t *testing.T) {
	pe := NewPolicyEngine()
	pe.AddPolicy(&SecurityPolicy{ID: "p1", Name: "t"})
	if err := pe.RemovePolicy("p1"); err != nil { t.Fatalf("Remove: %v", err) }
	if _, err := pe.GetPolicy("p1"); err == nil { t.Error("should be removed") }
	if err := pe.RemovePolicy("x"); err == nil { t.Error("remove nonexistent") }
}
...
func TestPolicyEngine_ListPolicies_Sorted(t *testing.T) {
	...
	p := pe.ListPolicies()
	if len(p) != 3 { t.Fatalf("want 3, got %d", len(p)) }
	if p[0].Priority != 10 || p[1].Priority != 20 || p[2].Priority != 30 { t.Error("not sorted") }
}
...
func TestPolicyEngine_Evaluate_ByNetwork(t *testing.T) {
	...
	r.IP = "10.0.0.1"
	if d := pe.Evaluate(r); d.Allowed { t.Error("external IP denied") }
}
...
func TestPolicyEngine_Evaluate_ByTimeRange(t *testing.T) {
	...
	if d := pe.Evaluate(r); d.Allowed { t.Error("off hours denied") }
}
...
func TestPolicyEngine_Evaluate_ByResourcePattern(t *testing.T) {
	...
	r.Resource = "/admin/x"
	if d := pe.Evaluate(r); d.Allowed { t.Error("/admin/x denied") }
}
...
func TestDeviceTrustManager_RegisterAndGet(t *testing.T) {
	...
	if err := dtm.RegisterDevice(d); err != nil { t.Fatalf("register: %v", err) }
...
func TestDeviceTrustManager_RegisterValidation(t *testing.T) {
	dtm := NewDeviceTrustManager()
	if err := dtm.RegisterDevice(&DeviceInfo{Fingerprint: "fp"}); err == nil { t.Error("reject empty ID") }
	if err := dtm.RegisterDevice(&DeviceInfo{ID: "d"}); err == nil { t.Error("reject empty fp") }
}

func TestDeviceTrustManager_DuplicateFingerprint(t *testing.T) {
	dtm := NewDeviceTrustManager()
	dtm.RegisterDevice(&DeviceInfo{ID: "d1", Name: "d1", Fingerprint: "fp"})
	if err := dtm.RegisterDevice(&DeviceInfo{ID: "d2", Name: "d2", Fingerprint: "fp"}); err == nil { t.Error("dup") }
	if err := dtm.RegisterDevice(&DeviceInfo{ID: "d1", Name: "d1u", Fingerprint: "fp"}); err != nil { t.Error("re-reg ok") }
}

func TestDeviceTrustManager_Unregister(t *testing.T) {
	dtm := NewDeviceTrustManager()
	dtm.RegisterDevice(&DeviceInfo{ID: "d1", Name: "d", Fingerprint: "fp"})
	if err := dtm.UnregisterDevice("d1"); err != nil { t.Fatalf("unreg: %v", err) }
	if _, err := dtm.GetDevice("d1"); err == nil { t.Error("should be gone") }
	if err := dtm.UnregisterDevice("x"); err == nil { t.Error("unreg nonexistent") }
}
...
func TestGenerateFingerprint(t *testing.T) {
	...
	if fp1 != fp2 { t.Error("order should not matter") }
	if len(fp1) != 64 { t.Errorf("len: %d", len(fp1)) }
	fp3 := GenerateFingerprint(map[string]string{"os": "windows"})
	if fp1 == fp3 { t.Error("different should differ") }
}
...
func TestListDevices(t *testing.T) {
	...
	d := dtm.ListDevices()
	if len(d) != 3 { t.Fatalf("want 3") }
	if d[0].ID != "a" || d[2].ID != "c" { t.Error("not sorted") }
}
...
func TestContinuousAuth_EmptyUser(t *testing.T) {
	ca := NewContinuousAuth()
	if _, err := ca.CreateSession("", "d", "1.2.3.4", "l"); err == nil { t.Error("reject empty") }
}

func TestContinuousAuth_EndSession(t *testing.T) {
	ca := NewContinuousAuth()
	s, _ := ca.CreateSession("u1", "d1", "1.2.3.4", "l")
	if err := ca.EndSession(s.ID); err != nil { t.Fatalf("end: %v", err) }
	got, _ := ca.GetSession(s.ID)
	if got.Active { t.Error("should be ended") }
	if err := ca.EndSession("x"); err == nil { t.Error("end nonexistent") }
}
...
func TestContinuousAuth_RecordActivity_RiskScore(t *testing.T) {
	...
	got, _ := ca.GetSession(s.ID)
	if got.RiskScore <= 0 { t.Error("risk should rise") }
}

func TestContinuousAuth_BlockedIP(t *testing.T) {
	ca := NewContinuousAuth()
	for i := 0; i < 5; i++ { ca.RecordFailedAttempt("10.0.0.1") }
	if _, err := ca.CreateSession("u", "d", "10.0.0.1", "l"); err == nil { t.Error("blocked IP") }
}
...
func TestContinuousAuth_GetUserSessions(t *testing.T) {
	...
	if len(ca.GetUserSessions("u1")) != 2 { t.Error("want 2") }
	if len(ca.GetUserSessions("nobody")) != 0 { t.Error("want 0") }
}
...
func TestContinuousAuth_CleanupExpired(t *testing.T) {
	...
	ca.mu.Unlock()
	if c := ca.CleanupExpiredSessions(); c != 1 { t.Errorf("want 1, got %d", c) }
}
...
func TestContinuousAuth_RecordOnEndedSession(t *testing.T) {
	...
	s, _ := ca.CreateSession("u1", "d1", "1.2.3.4", "l")
	ca.EndSession(s.ID)
	if err := ca.RecordActivity(s.ID, Activity{Type: "a", Success: true}); err == nil { t.Error("ended") }
}
...
func TestThreatDetector_DetectBruteForce(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		if ev := td.DetectBruteForce("10.0.0.1", false); ev != nil { t.Error("4 fails") }
	}
	ev := td.DetectBruteForce("10.0.0.1", false)
	if ev == nil { t.Fatal("5 fails trigger") }
	if ev.Severity != SeverityHigh { t.Errorf("sev: %s", ev.Severity) }
	if ev.Action != ActionBlock { t.Errorf("act: %s", ev.Action) }
}

func TestThreatDetector_DetectBruteForce_SuccessDoesNotTrigger(t *testing.T) {
	td := NewThreatDetector()
	for i := 0; i < 10; i++ { td.DetectBruteForce("10.0.0.1", true) }
	if ev := td.DetectBruteForce("10.0.0.1", true); ev != nil { t.Error("success") }
}

func TestThreatDetector_DetectAbnormalLogin(t *testing.T) {
	td := NewThreatDetector()
	if ev := td.DetectAbnormalLogin("u1", "1.2.3.4", "bj"); ev != nil { t.Error("1 loc") }
	if ev := td.DetectAbnormalLogin("u1", "5.6.7.8", "sh"); ev != nil { t.Error("2 loc") }
	ev := td.DetectAbnormalLogin("u1", "9.10.11.12", "gz")
	if ev == nil { t.Fatal("3 loc trigger") }
	if ev.Severity != SeverityHigh { t.Errorf("sev: %s", ev.Severity) }
}

func TestThreatDetector_DetectSQLInjection(t *testing.T) {
	td := NewThreatDetector()
	if ev := td.DetectSQLInjection("hello", "w"); ev != nil { t.Error("normal") }
	tests := []struct{ i, n string }{{"' OR '1'='1", "or"}, {"1; DROP TABLE x", "drop"}, {"1 UNION SELECT * FROM x", "union"}}
	for _, tc := range tests {
		ev := td.DetectSQLInjection(tc.i, "w")
		if ev == nil { t.Errorf("miss: %s", tc.n) }
		if ev != nil && ev.Severity != SeverityCritical { t.Errorf("sev: %s", ev.Severity) }
	}
}

func TestThreatDetector_DetectXSS(t *testing.T) {
	td := NewThreatDetector()
	if ev := td.DetectXSS("normal", "w"); ev != nil { t.Error("normal") }
	tests := []struct{ i, n string }{{"<script>x</script>", "script"}, {"javascript:x", "js"}, {"<img onerror=x>", "onerror"}, {"eval(x)", "eval"}}
	for _, tc := range tests {
		ev := td.DetectXSS(tc.i, "w")
		if ev == nil { t.Errorf("miss: %s", tc.n) }
		if ev != nil && ev.Severity != SeverityHigh { t.Errorf("sev: %s", ev.Severity) }
	}
}
...
func TestThreatDetector_GetEvents(t *testing.T) {
	...
	if len(td.GetEvents(10)) != 2 { t.Error("want 2") }
	if len(td.GetEventsByType("sql_injection", 10)) != 1 { t.Error("want 1") }
}
...
func TestSecurityEventManager_RecordAndGet(t *testing.T) {
	...
	if len(sem.GetEvents(10, nil)) != 1 { t.Error("want 1") }
}
...
func TestSecurityEventManager_HighSeverityCreatesAlert(t *testing.T) {
	...
	if len(sem.GetAlerts(10, nil)) != 1 { t.Error("want 1 alert") }
}
...
func TestSecurityEventManager_ResolveEvent(t *testing.T) {
	...
	if err := sem.ResolveEvent("ev1", "admin"); err != nil { t.Fatalf("resolve: %v", err) }
	events := sem.GetEvents(10, nil)
	if !events[0].Resolved { t.Error("resolved") }
	if events[0].ResolvedBy != "admin" { t.Errorf("by: %s", events[0].ResolvedBy) }
	if events[0].ResolvedAt == nil { t.Error("at nil") }
	if err := sem.ResolveEvent("x", "a"); err == nil { t.Error("nonexistent") }
}
...
func TestSecurityEventManager_GetEventStats(t *testing.T) {
	...
	if st["total"] != 3 { t.Errorf("total: %d", st["total"]) }
	if st["high"] != 1 { t.Errorf("high: %d", st["high"]) }
	if st["critical"] != 1 { t.Errorf("crit: %d", st["critical"]) }
}
...
func TestComplianceReporter_GenerateReport(t *testing.T) {
	...
	if report.Summary.ComplianceScore <= 0 { t.Error("score > 0") }
}
...
func TestComplianceReporter_ExportJSON(t *testing.T) {
	...
	if len(data) == 0 { t.Error("empty") }
}

func TestComplianceReporter_Recommendations(t *testing.T) {
	ztm := NewZeroTrustManager()
	report := ztm.Reporter.GenerateReport("test", time.Now(), time.Now())
	if len(report.Recommendations) == 0 { t.Error("should have recs") }
}
...
func TestResponseAction_String(t *testing.T) {
	...
	for _, tc := range tests {
		if tc.a.String() != tc.s { t.Errorf("%d: %s != %s", tc.a, tc.a.String(), tc.s) }
	}
}

func TestPolicyEffect_String(t *testing.T) {
	tests := []struct{ e PolicyEffect; s string }{
		{PolicyAllow, "allow"}, {PolicyDeny, "deny"}, {PolicyChallenge, "challenge"}, {PolicyEffect(99), "unknown"},
	}
	for _, tc := range tests {
		if tc.e.String() != tc.s { t.Errorf("%d: %s != %s", tc.e, tc.e.String(), tc.s) }
	}
}
...
func TestSecurityEventManager_SeverityFilter(t *testing.T) {
	...
	if len(events) != 2 { t.Errorf("want 2, got %d", len(events)) }
}

func TestContinuousAuth_HighRisk_KillsSession(t *testing.T) {
	ca := NewContinuousAuth()
	s, _ := ca.CreateSession("u1", "d1", "1.2.3.4", "l")
	for i := 0; i < 10; i++ { ca.RecordActivity(s.ID, Activity{Type: "access", Success: false, Location: "l1", IP: "1.2.3.4"}) }
	for i := 0; i < 5; i++ { ca.RecordActivity(s.ID, Activity{Type: "admin", Success: true, Location: "l2", IP: "5.6.7.8"}) }
	for i := 0; i < 5; i++ { ca.RecordActivity(s.ID, Activity{Type: "download", Success: true, Location: "l3", IP: "9.10.11.12"}) }
	for i := 0; i < 5; i++ { ca.RecordActivity(s.ID, Activity{Type: "admin", Success: false, Location: "l4", IP: "13.14.15.16"}) }
	got, _ := ca.GetSession(s.ID)
	if got.RiskScore >= 80 && got.Active { t.Error("high risk kill") }
}
