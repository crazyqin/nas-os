...
func TestVolumeMount_Structure(t *testing.T) {
	...
	assert.Equal(t, "/container/data", mount.Destination)
...
func TestContainerStats_Structure(t *testing.T) {
	...
	assert.Equal(t, 2.5, stats.CPUUsage)
	assert.Equal(t, uint64(52428800), stats.MemUsage)
	assert.Equal(t, 50.0, stats.MemPercent)
	assert.Equal(t, uint64(12), stats.PIDs)
}
...
func TestContainerConfig_Structure(t *testing.T) {
	config := ContainerConfig{
		...
		Volumes: []string{"/host/html:/usr/share/nginx/html"},
		Environment: map[string]string{
			"TZ": "Asia/Shanghai",
		},
		Network:  "bridge",
	...
	assert.Equal(t, "nginx-server", config.Name)
...
func TestContainerLog_Stderr(t *testing.T) {
	log := ContainerLog{
		Timestamp: time.Now(),
		Line:      "Warning: deprecated API usage",
		Source:    "stderr",
	}
