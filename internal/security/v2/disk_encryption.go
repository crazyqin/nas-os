...
func (dm *DiskEncryptionManager) CreateLUKS(devicePath, passphrase string, config *DiskEncryptionConfig) error {
	...
	cmd := exec.CommandContext(context.Background(), "cryptsetup", "-q", "--type", luksVersion,
		"--cipher", cipher,
...
func (dm *DiskEncryptionManager) OpenLUKS(devicePath, mapperName, passphrase string) error {
	...
	// 使用 cryptsetup 打开卷
	cmd := exec.CommandContext(context.Background(), "cryptsetup", "luksOpen", devicePath, mapperName)
...
func (dm *DiskEncryptionManager) CloseLUKS(mapperName string) error {
	...
	// 使用 cryptsetup 关闭卷
	cmd := exec.CommandContext(context.Background(), "cryptsetup", "luksClose", mapperName)
