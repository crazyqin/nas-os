...
const (
	...
	StorageClassInfreq   StorageClass = "infrequent_access"
...
type BucketQuota struct {
	MaxSize    int64 `json:"maxSize"`    // 最大容量(字节), 0=无限制
	MaxObjects int64 `json:"maxObjects"` // 最大对象数, 0=无限制
}
...
type LifecycleRule struct {
	...
	ExpirationDays  int           `json:"expirationDays"`  // 过期天数, 0=不过期
	TransitionDays  int           `json:"transitionDays"`  // 转换天数
	TargetClass     StorageClass  `json:"targetClass"`     // 目标存储类别
}
