#!/bin/bash
# =============================================================================
# NAS-OS v2.125.0 部署脚本
# =============================================================================
# 用途：版本专用部署脚本，支持二进制部署、Docker 部署、升级、回滚
# 用法：./deploy-v2.125.0.sh <command> [options]
#
# v2.125.0 更新（工部）：
# - 版本专用部署脚本
# - 优化部署流程和错误处理
...
log_deployment() {
    ...
    mkdir -p "$LOG_DIR"
    echo "[$(date -Iseconds)] [v${VERSION}] [$action] $details" >> "$log_file"
}
...
pre_deploy_check() {
    ...
    if command -v btrfs &>/dev/null; then
        log_info "Btrfs 工具: 已安装"
    else
        log_warn "Btrfs 工具未安装，部分存储功能可能受限"
    fi
    
    # 检查 SMART 工具
    if command -v smartctl &>/dev/null; then
        log_info "SMART 工具: 已安装"
    else
        log_warn "SMART 工具未安装，磁盘健康监控功能受限"
    fi
    
    # 检查 Docker（可选）
    if command -v docker &>/dev/null; then
        log_info "Docker: 已安装 ($(docker --version 2>/dev/null || echo 'unknown'))"
    fi
...
create_directories() {
    ...
    run_cmd mkdir -p "$DATA_DIR/cache"
    run_cmd mkdir -p "$DATA_DIR/certs"
...
backup_current() {
    ...
    local backup_name="nasd-v${VERSION}-$(date +%Y%m%d-%H%M%S).bak"
    ...
    if [ -d "$CONFIG_DIR" ]; then
        tar -czf "$BACKUP_DIR/config-v${VERSION}-$(date +%Y%m%d-%H%M%S).tar.gz" -C "$(dirname "$CONFIG_DIR")" "$(basename "$CONFIG_DIR")" 2>/dev/null || true
    fi
...
[Unit]
Description=NAS-OS - Home NAS Management System (v${VERSION})
...
Environment=NAS_OS_VERSION=${VERSION}
...
deploy_docker() {
    log_step "使用 Docker 部署 v${VERSION}..."
...
# 升级
do_upgrade() {
    log_step "开始升级 NAS-OS 到 v${VERSION}..."
    ...
    if install_binary; then
        if start_service && health_check; then
            post_deploy_verify
            log_deployment "upgrade" "成功升级到 v${VERSION}"
            log_success "升级完成"
        else
...
show_help() {
    cat <<EOF
NAS-OS v${VERSION} 部署工具

用法: $0 <command> [options]

命令:
  install     安装 NAS-OS v${VERSION}（二进制方式）
  docker      使用 Docker 部署 v${VERSION}
...
示例:
  $0 install           # 安装 v${VERSION}
  $0 upgrade           # 升级到 v${VERSION}
  $0 rollback          # 回滚到上一个版本
  $0 --dry-run install # 模拟安装
  $0 docker            # Docker 部署 v${VERSION}
EOF
}
