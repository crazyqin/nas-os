...
- 修复 CI 编译错误（undefined: RansomwareDetector, DetectionRule）
- 实现检测规则管理、活动记录、统计信息
- 支持熵值检测、批量写入、快速删除、可疑扩展名四种检测规则

### 竞品功能对标
- 对标 TrueNAS 26：分布式共识引擎、勒索软件检测增强
- 对标群晖 DSM 7.4：不可变审计日志、安全合规增强
- 对标飞牛 fnOS：系统稳定性提升
