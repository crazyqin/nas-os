...
#### 🔀 Hybrid Flash Pool管理
- 混合存储池管理（NVMe + SSD + HDD）
- 智能数据分层与自动迁移
- 闪存加速读写缓存策略
- 对标 TrueNAS 26 Hybrid Flash Pool

#### 🔒 不可变快照防勒索
- 不可变快照保护数据安全
- 防勒索软件加密攻击
- 快照保留策略与合规管理
- 对标群晖 Immutable Snapshots

#### ⏰ 定时ZFS Scrub调度
- 自动化ZFS数据完整性检查
- 可配置的定时Scrub任务
- Scrub结果分析与告警
- 对标 TrueNAS 26 Scheduled Scrubbing
...
#### 修复
- 修复 assetmgr GetAssetSummary groups计数
- 修复 cloudsync 测试字段引用和空数组返回
- 修复 clustermanager/containersec/costgovernance/migration 测试
- 新增 filecache 缓存管理器
- 新增 taskqueue DAG任务编排
...
#### 修复
- 修复 surveillance 测试编译错误（缺少 :=）
- 修复 assetmgr 测试断言错误（groups 期望值）
- 修复 cloudsync 测试状态码与响应格式不匹配

---

## [v2.499.0] - 2026-05-27

### 竞品对标：ZFS池管理 / Docker Compose GUI / GPU监控 / 虚拟机管理 / 系统仪表盘

- **ZFS存储池管理**: 完整的ZFS池/数据集/快照管理，支持RAID-Z Expansion、清洗、回滚
- **Docker Compose GUI**: 容器/镜像/网络管理，支持Compose项目部署、日志查看、资源监控
- **GPU监控**: 多厂商GPU支持，实时温度/功耗/显存监控，进程追踪，历史统计
- **虚拟机管理**: KVM虚拟化管理，支持GPU/USB直通、快照、暂停恢复、UEFI/TPM
- **系统仪表盘**: 可自定义仪表盘，多种组件类型，告警管理，服务状态监控

## [v2.494.0] - 2026-05-27

### 六大新模块：统一搜索 / 能源管理 / 隐私保险库 / 数据分类 / 网络拓扑 / 智能家居 / 协作文档 / 数字健康 / 边缘计算 / 照片增强

#### 新功能

##### 🔍 统一搜索 (Unified Search)
- 跨平台全文搜索：文件/照片/邮件/笔记/日历/联系人
- 多种搜索模式：关键词/语义/模糊/正则表达式
- 搜索结果分面过滤（Faceted Search）
- 支持高亮显示和搜索建议
- 索引统计与健康监控
- 竞品对标：超越群晖 Universal Search
...
- 安全删除（多次覆写）
- 完整审计日志
- 竞品对标：超越 TrueNAS 加密能力

##### 🏷️ 数据分类 (Data Classification)
- AI 驱动自动分类（公开/内部/机密/限制/绝密）
- PII 检测（邮箱/电话/身份证/信用卡/护照）
- 自动标签与关键词提取
- 分类规则引擎
- 合规评分统计
- 竞品对标：超越群晖 AI 分类
...
##### 🏠 智能家居 (Smart Home)
- Matter/HomeKit/Zigbee/Z-Wave/MQTT 多协议支持
- 设备管理与房间配置
- 场景与自动化
- 竞品对标：超越群晖 Home Assistant 集成
...
##### 🖼️ 照片增强 (Photo Enhance)
- AI 照片修复与增强
- 超分辨率放大
- 去噪与去模糊

#### Bug 修复
- 修复 photoenhance 测试中未使用的 context 导入
