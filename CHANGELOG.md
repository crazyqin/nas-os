...
#### 智能存储成本优化 (smartstoragecostopt)
- 存储成本分析引擎
- 多层级存储画像（NVMe/SSD/HDD/云/磁带）
- ROI分析与趋势预测
- 预算告警机制
...
- 对标飞牛fnOS：AI相册、应用商店
- 对标群晖DSM：存储成本分析、监控告警
- 对标TrueNAS：ZFS增强、数据安全

---

## v2.608.0 (2026-06-20) - 存储数据分层/AI媒体智能/应用商店增强/远程访问NAT穿透

### 新增功能
- **存储数据分层管理 (storage/tiering)** - 对标群晖 Synology Tiering，支持冷热数据自动迁移、多层级存储策略配置（热/温/冷/归档四层架构）、负载均衡和健康监控、智能数据放置推荐
- **AI媒体智能分析器 (mediaai/analyzer)** - 对标飞牛fnOS相册AI识别和群晖Synology Photos，支持场景分类与物体检测、人脸检测与识别、内容质量评估、NSFW内容检测、自动标签生成、批量分析、相似媒体搜索
- **应用商店评论系统 (appstore/review)** - 用户评分评论、回复互动、评分统计、匿名评论控制、安装后才能评论等策略
- **应用自动更新管理 (appstore/updater)** - 更新策略配置、定时检查、批量更新、回滚机制、更新通知、预更新备份
- **增强依赖解析 (appstore/dependency)** - 语义化版本约束解析（semver）、智能冲突检测、依赖图分析、版本比较
- **远程访问NAT穿透 (remoteaccess/)** - P2P连接管理、STUN协议、中继服务器、带宽自适应、加密隧道、Hole Punching技术
- **存储成本分析器 (storagecostanalyzer)** - 多维度存储成本对比、TCO报告生成、优化建议

### 竞品对标
- 对标群晖DSM 7.3：存储数据分层（Synology Tiering）、AI协作功能、安全增强
- 对标飞牛fnOS：AI媒体智能管理、相册AI识别、ARM架构支持
- 对标TrueNAS 26：存储分层优化、年度发布节奏简化
...
### 新增功能
- **块级去重2.0 (blockdedup2)** - 支持BRT(Block Reference Table)的块级去重引擎，SHA-256哈希计算，引用计数与去重统计，HTTP API处理器
- **AI工作流可视化 (aiworkflowviz)** - 可视化工作流编辑器，支持多节点类型（输入/输出/处理/AI模型/过滤/合并/拆分），节点拖拽定位，边连接管理，工作流状态管理
- **智能分层2.0 (smarttier2)** - 四层存储架构（热/温/冷/归档），访问频率分析，智能迁移推荐，数据分类与统计
- **GPU加速推理2.0 (gpuaccel2)** - GPU设备管理与监控，模型加载/卸载，推理请求处理，内存与性能统计
- **WebShare Pro 2.0 (websharepro2)** - 多种分享类型（链接/用户/组/公开），权限管理，访问日志，协作会话，下载统计
- **ML勒索防护2.0 (ransomml2)** - 机器学习异常检测，高熵值文件检测，批量操作监控，威胁等级分类，自动阻断与告警

### 竞品对标
- 对标群晖 COMPUTEX 2026：GPU NAS支持、AI工作流编排、DSM Agent增强
- 对标 TrueNAS 26：OpenZFS 2.4混合池、WebShare、LXC容器HA
- 对标飞牛 fnOS：企业级ACL、AMD GPU适配、智能分层
...
- **飞牛 fnOS**: AMD GPU硬件加速全面支持 + ARM架构公测版

### 优化
- 更新竞品分析文档，补充群晖COMPUTEX 2026最新产品线
- 完善MCP协议集成能力，与群晖AI Console对齐
- 增强事件驱动架构，支持企业级工作流自动化
...
### 优化
- 更新VERSION至v2.601.0
- 优化CI/CD工作流配置，提升构建稳定性
- 更新竞品分析文档，补充飞牛/群晖/TrueNAS最新功能对比
- 增强资源监控统计，提供更详细的系统指标
- 完善项目管理流程，优化版本发布机制
- 加强安全检查，提升系统安全性
...
### 新增功能
- **AI相册引擎 (aiphotoalbum)** - 对标飞牛fnOS AI相册功能，支持人脸识别、以文搜图、智能分类、时间轴视图、人物聚合、标签管理、相册共享（对标飞牛fnOS AI相册）
- **智能文件标签系统 (smartfiletags)** - 对标群晖DSM 7.3 Drive共享标签，支持标签创建/更新/删除、文件打标、智能分类、标签搜索、自动标签规则、标签建议、用户标签管理（对标群晖Drive共享标签）
- **智能数据分层引擎 (smarttiering)** - 对标群晖DSM 7.3 Synology Tiering，支持热/温/冷/归档四级存储、自动分层策略、热度分析、访问模式识别、批量迁移、存储优化（对标群晖Synology Tiering）

### 竞品分析
- 飞牛 fnOS: AI相册人脸识别、以文搜图、FN Connect远程访问、应用中心
- 群晖 DSM 7.3: Synology Tiering智能分层、Drive共享标签、AI Console、数据脱敏
- TrueNAS 25.04: ZFS增强、RAID-Z Expansion、Fast-Dedup、SMB多通道

### 测试
- AI相册: 20个测试用例（引擎创建/启停/添加删除照片/文本搜索/人物照片/标签照片/相册CRUD/人脸识别/统计/日期过滤/标签过滤/多人脸/向量索引/配置/相册共享/GPS/并发）
- 智能标签: 21个测试用例（引擎创建/启停/标签CRUD/按名获取/列出/应用移除/搜索/建议/自动标签/规则/统计/多文件/用户标签/时间戳/公开私有/系统标签/配置）
- 智能分层: 24个测试用例（引擎创建/启停/存储层CRUD/策略CRUD/文件CRUD/访问记录/热度分数/热文件/按层获取/分析迁移/执行迁移/迁移队列/统计/配置/层类型/多次访问/固定文件/禁用自动/并发）
