...
| 模块 | 功能 | 核心文件 |
|------|------|----------|
| `storage` | BTRFS 存储管理 | manager.go |
| `smb` | SMB/CIFS 文件共享 | manager.go, handlers.go |
| `nfs` | NFS 文件共享 | manager.go, handlers.go |
| `docker` | Docker 容器管理 | manager.go, appstore.go |
| `cluster` | 集群支持 | manager.go, ha.go |
| `users` | 用户管理 | manager.go |
| `auth` | 认证授权 | middleware.go |
| `backup` | 备份恢复 | manager.go |
| `monitor` | 系统监控 | health.go, alerts.go |
| `quota` | 配额管理 | manager.go |
| `trash` | 回收站 | manager.go |
| `webdav` | WebDAV 服务 | manager.go |
| `replication` | 存储复制 | manager.go |
| `cache` | 缓存系统 | manager.go |
| `search` | 智能搜索 | manager.go |
| `photos` | 照片管理 | manager.go |
| `media` | 媒体服务 | manager.go |
| `vm` | 虚拟机管理 | manager.go |
| `prediction` | 预测分析 | manager.go |
