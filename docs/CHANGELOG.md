...
- **备份项目管理器**
  ...
  - 任务追踪系统 (TaskTracker) - 任务状态流转、事件记录
  - 进度统计系统 (ProgressStats) - 任务统计、进度追踪
  - 数据持久化 - JSON 格式保存/加载
  - 报告生成 - 备份进度报告、统计汇总

### 🔧 核心功能

- **备份项目管理**: 项目创建/更新/删除/备份/恢复
- **任务追踪**: 任务状态管理（pending/running/completed/failed）
- **进度统计**: 总任务数、完成数、失败数、总体进度
- **事件记录**: 任务事件日志，支持最多 1000 条记录
- **数据持久化**: 自动保存/加载任务数据
- **报告生成**: 生成备份进度统计报告
