...
NAS-OS v2.442.0+ 提供图形化的FRP内网穿透Web管理界面，对标飞牛FN Connect用户体验。
...
| 类型 | 适用场景 | 访问方式 |
|------|---------|---------|
| HTTP | Web服务暴露 | `https://subdomain.connect.fnos.cn` |
| HTTPS | 安全Web服务 | 自定义域名+TLS证书 |
| TCP | SSH/数据库等 | `tcp://node:port` |
| UDP | 游戏/流媒体 | `udp://node:port` |
